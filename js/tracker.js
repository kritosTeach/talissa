...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('submit', (e) => {
        ...
        // Wait 500ms to ensure backend receives the event, then redirect or reset
        setTimeout(() => {
            if (redirectUrl) {
                // Redirect to the specified URL
                window.location.href = redirectUrl;
            } else if (form.action && new URL(form.action, window.location.origin).pathname !== window.location.pathname) {
                window.location.href = form.action;
            } else {
                // Reset form if no redirect needed
                if (submitBtn) {
        ...
        }, 500); // Give the backend 500ms to record and broadcast the event before redirect
    });
